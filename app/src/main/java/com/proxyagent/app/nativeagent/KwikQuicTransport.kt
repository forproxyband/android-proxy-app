...
class KwikQuicTransport private constructor(
    ...
    companion object {
        ...
        // SendBuffer cap patch was attempted (build 75) and reverted:
        // lifting kwik's 50 KB per-stream SendBuffer.maxBufferSize to
        // 4 MiB DID let the application thread write further ahead,
        // but caused a worse regression in the receive direction —
        // upload throughput collapsed 96 → 23 Mbps in a speedtest.
        // Most likely cause: kwik's single sender thread spent more
        // wall time draining the bigger STREAM-frame backlog, which
        // delayed the connection's MAX_STREAM_DATA / MAX_DATA window
        // updates back to the peer. With a stale flow-control window
        // the peer's stack throttled its own sends to us, gating the
        // direction we were receiving on. The 50 KB default acts as
        // implicit fairness between the sender's STREAM-frame work
        // and its ACK/window-update work — a knob worth more careful
        // investigation than a one-line patch.
